...
const $$InteriorNav = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<header class="interior-nav fixed top-0 left-0 right-0 z-50 h-14 flex items-center px-6 md:px-10"> <!-- Back link --> <a href="${import.meta.env.BASE_URL}" class="font-display text-xs uppercase tracking-widest text-ink-muted hover:text-gold transition-colors duration-200 flex items-center gap-2 shrink-0"> <span aria-hidden="true" class="text-gold-pale">←</span> <span class="hidden sm:inline">Back to the Map</span> <span class="sm:hidden">Map</span> </a> <!-- Page title, centered absolutely so it doesn't depend on siblings --> ${pageTitle && renderTemplate`<h1 class="font-decorative text-xs uppercase tracking-[0.2em] text-gold absolute left-1/2 -translate-x-1/2 pointer-events-none whitespace-nowrap"> ${pageTitle} </h1>`} </header>`;
